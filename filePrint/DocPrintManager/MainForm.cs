...
namespace DocPrintManager
{
    public partial class MainForm : Form
    {
        ...
        {
            var printedDocs = new BindingList<DocumentGridItem>(GridData.Where(s => s.Printed.HasValue && s.Printed.Value).ToList());
            if (!printedDocs.Any()) return;
            var form = new PrintedDocsInfoForm(printedDocs);
            form.ShowDialog();
        }
        ...
        private void _btnGetAvgPrintTime_Click(object sender, EventArgs e)
        {
            ...
            if (avgPrintTime.HasValue) MessageBox.Show($"{avgPrintTime:0.##} сек.", "Средняя продолжительность печати");
        }
    }
}
